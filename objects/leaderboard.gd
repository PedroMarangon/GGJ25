...
func _update_leaderboard(delta):
	...
	for player_rank in bubble_rank:
		...
		if referenceLabels[player_id].position.distance_to(referenceVboxPositions[player_rank]) > 1 and referenceLabels[player_id].position.y > referenceVboxPositions[player_rank].y:
			referenceLabels[player_id].label_settings.shadow_size = lerp(referenceLabels[player_id].label_settings.shadow_size, 35, 0.2)
		else:
			referenceLabels[player_id].label_settings.shadow_size = lerp(referenceLabels[player_id].label_settings.shadow_size, 4, 0.2)
